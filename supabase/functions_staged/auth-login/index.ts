...
Deno.serve(async (req: Request) => {
  ...
  // Tenant claim: every RLS policy keys off app_metadata.guild_id.
  const { data: info, error: iErr } = await admin.rpc("gm_account_info", { p_id: id });
  const infoRow = Array.isArray(info) ? info[0] : info;
  if (iErr || !infoRow?.guild_id) return json({ ok: false, error: "server_error" }, 500);

  const meta = { app_role: role, account_id: id, guild_id: infoRow.guild_id };
