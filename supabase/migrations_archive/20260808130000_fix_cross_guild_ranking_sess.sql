-- 20260808130000_fix_cross_guild_ranking_sess.sql
-- gm_cross_guild_ranking built sess as DISTINCT (guild, event_name, skey), so
-- Arms Race Stage A and Stage B of the same week kept two rows with the same
-- scoring key, over-counting Arms (and inflating global totals). sess is now
-- DISTINCT (guild, skey) and the per-type totals/counters classify rows by the
-- scoring key prefix instead of the raw event_name. Fresh OID.
