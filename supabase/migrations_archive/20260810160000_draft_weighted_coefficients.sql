-- 20260810160000_draft_weighted_coefficients.sql
-- Applies weighted coefficients to calculate overall global participation rate in Draft ranking:
-- SvS (weight 5), GvG (weight 5), Shadowfront (weight 3), DTR (weight 2), Arms Race (weight 2).
...
begin
  ...
  SELECT r.pseudo,
         ...
         -- Weighted global rate: SvS (weight 5), GvG (weight 5), Shadowfront (weight 3), DTR (weight 2), Arms Race (weight 2)
         CASE
           WHEN (
             (CASE WHEN COALESCE(st.svs_tot, 0) > 0 THEN 5 ELSE 0 END) +
             (CASE WHEN COALESCE(st.gvg_tot, 0) > 0 THEN 5 ELSE 0 END) +
             (CASE WHEN COALESCE(st.sh_tot, 0)  > 0 THEN 3 ELSE 0 END) +
             ...
             round(
               (
                 ...
                 (CASE WHEN COALESCE(st.sh_tot, 0)  > 0 THEN 3.0 * COALESCE(ps.sh_att, 0)  / st.sh_tot  ELSE 0 END) +
                 ...
                 (CASE WHEN COALESCE(st.sh_tot, 0)  > 0 THEN 3.0 ELSE 0 END) +
