-- 20260808040000_gm_list_event_sessions_sort_by_week.sql
-- gm_list_event_sessions ordered by min(session_id::timestamptz), but
-- session_id is no longer a timestamp (ARA-20260809, SVS-2026-W32, ...) so
-- the cast would raise. Session-less events (Glory) and sessioned events now
-- sort by week_start (Monday) when no battle date was chosen. New OID applied
-- so PostgREST drops any cached plan.
