-- 20260808000000_gm_list_event_sessions_group_by_week.sql
-- gm_list_event_sessions groups by (event, session). Glory (and any other
-- session-less event) shares session_id = NULL, so 4 weeks of Glory collapse
-- into a single History row. New OID (drop + create) so PostgREST drops any
-- cached plan of the old body. Group key now uses coalesce(session_id,
-- week_start): sessioned events stay one row per session, session-less events
-- (Glory) get one row per week.
...
begin
    ...
    -- par week_start via coalesce. Le squad1 avait des week_start mixtes, ce
    -- qui créait des doublons : le coalesce(session_id) les préserve en une
    -- seule ligne par session.
