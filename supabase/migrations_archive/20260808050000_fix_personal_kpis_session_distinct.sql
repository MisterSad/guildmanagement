-- 20260808050000_fix_personal_kpis_session_distinct.sql
-- gm_personal_kpis counted every event_participants ROW (per pseudo), so a
-- player who appeared in two sessions of the same event was counted twice in
-- both the total and the attendance. With duplicate sessions now fused and
-- Glory keyed by session_id, the rates count DISTINCT sessions per event
-- type (still excluding Glory), matching the app-wide participation
-- semantics.
...
begin
  ...
  -- Participation: DISTINCT sessions (Glory excluded), overall and per type.
  select count(distinct ep.session_id),
         count(distinct ep.session_id) filter (where ep.participated > 0 or ep.sub_present)
  ...
  -- Guild average participation rate (per member, same session set).
  select round(avg(r.rate)) into v_guild_avg_rate
  from (
    select
      count(distinct ep.session_id) filter (where ep.participated > 0 or ep.sub_present)::numeric
        / nullif(count(distinct ep.session_id), 0) * 100 as rate
    from public.event_participants ep
    where ep.guild = v_member.guild
      and lower(ep.event_name) <> 'glory'
    group by ep.pseudo
  ...
  from (
    select
      ...
      count(distinct ep.session_id) as total,
      count(distinct ep.session_id) filter (where ep.participated > 0 or ep.sub_present) as attended
    from public.event_participants ep
    where ep.guild = v_member.guild
      and lower(ep.pseudo) = lower(v_member.pseudo)
      and lower(ep.event_name) <> 'glory'
