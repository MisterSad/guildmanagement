-- 20260810080000_fix_glory_history_participation_count.sql
-- Fixes Glory participation count in event history:
-- 1. Updates event_participants where score > 0 so participated is set to 1.
-- 2. Updates gm_list_event_sessions to count a participant if ep.participated > 0 OR ep.score > 0.

update public.event_participants
set participated = 1
where coalesce(score, 0) > 0 or coalesce(score_prep, 0) > 0 or coalesce(score_pvp, 0) > 0;
...
begin
    ...
    select
        ...
        sum(case when ep.participated > 0 or coalesce(ep.score, 0) > 0 or coalesce(ep.score_prep, 0) > 0 or coalesce(ep.score_pvp, 0) > 0 then 1 else 0 end)::integer as participated_count,
