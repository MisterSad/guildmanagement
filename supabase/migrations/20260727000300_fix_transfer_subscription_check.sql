-- Migration: Fix transfer_guild_member to check subscription status for R4 callers
-- ISSUE (I4): R4 admins from expired guilds could still trigger member transfers.
...
BEGIN
    ...
    -- FIX (I4): Check subscription is active for R4 callers
    IF v_caller_role <> 'R5' AND NOT public.is_subscription_active(v_source_guild) THEN
