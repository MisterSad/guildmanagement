-- ==============================================================================
-- MIGRATION: 20260818030000_demo_tenant_dynamic_data_and_daily_reset.sql
-- DESCRIPTION: Deterministic Dynamic DEMO Tenant Reset & Daily pg_cron Schedule
-- INVARIANTS: Multi-Tenant SaaS compliant, Zero-Trust RLS, Search Path Hardened
-- ==============================================================================

-- 1. Create public.gm_reset_demo_tenant_data()
...
BEGIN
    ...
    -- UID space 90000001 - 90000060 (guaranteed collision-free)
    FOR v_idx IN 1..60 LOOP
        ...
        -- Role and Military Metrics computation
        IF v_idx = 1 THEN
            v_role := 'R5';
            v_power := 168000000;
            v_tech := 58000000;
    ...
    FOREACH v_w IN ARRAY v_weeks LOOP
        ...
        FOR v_m IN (SELECT * FROM public.guild_members WHERE guild = 'DEMO' ORDER BY id ASC) LOOP
            v_idx := v_idx + 1;
            -- Progression drift (weeks in the past had slightly lower scores)
            ...
            -- C. SvS Event Participants (Dual scoring: Prep + PvP)
    ...
    -- Step 7: Shadowfront Squads & Signups for recent and current weeks
    FOREACH v_w IN ARRAY v_weeks LOOP
...
-- 2. Setup pg_cron daily job if pg_cron extension is available
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        -- Unschedule previous job if exists
        PERFORM cron.unschedule('daily-demo-tenant-reset')
        WHERE EXISTS (
            SELECT 1 FROM cron.job WHERE jobname = 'daily-demo-tenant-reset'
        );

        -- Schedule daily reset at 03:00 UTC
        PERFORM cron.schedule(
            'daily-demo-tenant-reset',
            '0 3 * * *',
            $job$ SELECT public.gm_reset_demo_tenant_data(); $job$
        );
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'pg_cron registration skipped or managed by cloud provider.';
END;
$$;

-- 3. Execute immediately to populate initial pristine dynamic dataset
