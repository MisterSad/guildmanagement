-- 20260808090000_fix_cross_guild_ranking_scoring_key.sql
-- gm_cross_guild_ranking keyed sessions by session_id, so Shadowfront Squad 1
-- and Squad 2 counted as two sessions per week, and Arms Race Stage A and B
-- as two sessions per week. Per the game rules Shadowfront and Arms Race are
-- one participation per week. The session key now uses
-- gm_event_scoring_key (Arms/Shadowfront keyed by week_start, DTR by session,
-- SvS/GvG by week), matching stats.js. Fresh OID.
