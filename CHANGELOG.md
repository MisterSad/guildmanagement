...
- **Active events showed players from other guilds**: with the new deterministic session ids, the same id (e.g. `GVG-2026-W32`) is shared by every guild running that event that week. Queries that only filtered by `event_name` + `session_id` therefore loaded every guild's participants, so ALPHA's GvG listed DEMO/BABE/... players too. Every read and write on `event_participants`, `event_status`, `shadowfront_squads` and `shadowfront_signups` now also filters by the active guild, and `gm_populate_event_participants` takes an explicit `p_guild` (enforced against the caller) instead of guessing the guild from the shared session id. Existing rows were verified clean, no cross-tenant data.
