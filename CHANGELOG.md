...
- **UI Layout: Sidebar Sub-card scroll fix**: Fixed a layout bug in the sidebar (`shell.css`) where a large nav menu would push the subscription card off-screen. Added `min-height: 0` to `.gm-sidebar-nav` and made the card sticky.
