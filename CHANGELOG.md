...
- **UI Layout: Sidebar Sub-card scroll fix**: Fixed a layout bug in the sidebar (`shell.css` v13) where the subscription card was incorrectly positioned relative to the nav items when scrolling on desktop. Changed `.gm-sidebar-sub-card` to use `position: absolute` at the bottom of the sticky sidebar container, ensuring it remains fixed at the bottom of the viewport while the nav items scroll independently.
