...
- **Supabase Edge Function Discord Proxy (`discord-webhook-proxy`)**: Deployed a dedicated server-side Deno Edge Function proxy that forwards Discord Webhook requests directly from Supabase infrastructure. This completely bypasses browser CORS restrictions, adblockers, and browser policy limits (`NetworkError when attempting to fetch resource`).
- **Discord Webhook Fallback & Robust Error Diagnostics**: Enhanced Discord Webhook integration for Super Admin Opponent Roster sharing with automatic URL formatting/cleaning, detailed HTTP status error reporting (e.g. 404/400/429), and server-side Edge Function proxy routing.
...
- **Bypass Browser NetworkError & CORS Block**: Solved `NetworkError when attempting to fetch resource` when calling Discord Webhooks from client JS by proxying calls through Supabase Edge Functions.
