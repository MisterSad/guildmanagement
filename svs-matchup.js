...
(function () {
                ...
                '<div style="position:relative; display:grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap:1.25rem; align-items:center;">' +
                    ...
                    '<div style="background:rgba(239, 68, 68, 0.08); border:1px solid rgba(239, 68, 68, 0.25); border-radius:12px; padding:.75rem 1rem;">' +
                        '<div style="font-weight:800; color:#f87171; font-size:0.8rem; text-transform:uppercase; letter-spacing:.05em; margin-bottom:.4rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:.4rem;">' +
                            '<span style="display:flex; align-items:center; gap:.4rem;"><i class="ph ph-crosshair"></i> Server B (Opponent / Target)</span>' +
                            '<button id="svs-share-discord-b" class="gm-btn gm-btn-primary gm-btn-sm" style="font-size:0.75rem; padding:0.2rem 0.5rem; display:inline-flex; align-items:center; gap:0.3rem; height:auto; line-height:1.2;">' +
            ...
            '<div class="gm-card glass-card" style="padding:.85rem; overflow:hidden;">' +
                '<div style="font-weight:800; font-size:0.9rem; color:#f87171; margin-bottom:.65rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:.4rem;">' +
                    ...
                    '<div style="display:flex; align-items:center; gap:.5rem;">' +
                        ...
                        '<button id="svs-share-discord-table-b" class="gm-btn gm-btn-secondary gm-btn-sm" style="font-size:0.72rem; padding:0.18rem 0.45rem; display:inline-flex; align-items:center; gap:0.25rem; height:auto; line-height:1.2;">' +
    ...
    function wireControls(container) {
        ...
        function handleShareSvS(btn) {
            ...
            var rowsB = sortRows(filterRows(state.serverB, state.guildB), state.sortKey, state.sortDesc);
            var label = 'Server ' + formatServerDisplay(state.serverB) + (state.guildB !== 'ALL' ? ' (' + state.guildB + ')' : '');
        ...
        var btnShareB = document.getElementById('svs-share-discord-b');
        if (btnShareB) btnShareB.addEventListener('click', function () { handleShareSvS(btnShareB); });

        var btnShareTableB = document.getElementById('svs-share-discord-table-b');
        if (btnShareTableB) btnShareTableB.addEventListener('click', function () { handleShareSvS(btnShareTableB); });
    }
