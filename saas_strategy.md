...
|---|----------|-------|----------|
| 0 | Sécurité immédiate | 1–2 j | Faille critique RLS en prod aujourd'hui (voir §4) |
| 1 | Multi-tenancy | 2–3 sem | Aucune table n'a de `guild_id` ; tout suppose UNE guilde |
| 2 | Auth & rôles | 1 sem | Mots de passe réversibles, pas de reset, rôles non vérifiés côté serveur |
| 3 | i18n extensible | 3–4 j | EN par défaut, registre de langues, fichiers de locale séparés |
...
| 6 | Configurabilité | 1–1,5 sem | Horaires de rappels codés en dur = inutilisables pour d'autres serveurs |
| 7 | Rebranding | 1 j | Inventaire précis en Annexe A |
| 8 | Landing page | 3–4 j | Page marketing moderne EN/FR, SEO |
| 9 | Onboarding | 3–4 j | Wizard de création de guilde + import CSV |
...
- [ ] Refondre les unicités : `UNIQUE (guild_id, pseudo)` sur `guild_members` ; `UNIQUE (guild_id, event_name)` sur `event_status` ; `guild_config` → PK `(guild_id, key)` ; `accounts` → `UNIQUE (guild_id, id)` (l'identifiant de login devient `slug/identifiant` ou l'unicité reste globale, à trancher — recommandation : **identifiant global unique** pour garder un écran de login sans champ guilde).
- [ ] Sortir les **locks de notifications** de `guild_config` vers une table dédiée `notification_locks (guild_id, lock_key, status, created_at)` avec TTL/purge — le mélange config/état actuel pollue la config.
...
- [ ] `auth-login` injecte `guild_id` + `app_role` dans `app_metadata` (le mécanisme existe déjà pour `app_role` — l'étendre).
...
- [ ] `event-reminders` : boucler sur les guildes **actives** (abonnement valide), lire le webhook Discord *de la guilde*, pousser **uniquement aux abonnés push de la guilde**, namespacer les locks par `guild_id`. ⚠️ Aujourd'hui un rappel partirait chez tous les clients à la fois.
...
4. **Détection de langue** à la première visite : `localStorage` → `navigator.languages` (premier match avec le registre) → `en`. Persistance : clé `localStorage` unique (renommée `gmt_lang`, avec migration de l'ancienne `rad_lang`).
...
7. **Tous les textes statiques du HTML passent en anglais** (valeurs par défaut avant traduction) — supprime le flash FR et aligne le défaut.
8. **Dates & nombres via `Intl`** avec mapping locale (`en → en-GB`, `fr → fr-FR`…) : remplacer les `'fr-FR'` en dur et `formatNumber` par `Intl.NumberFormat(locale)`. Les heures d'événements restent affichées **en UTC** (convention du jeu, à conserver — c'est un choix produit, pas un bug).
...
1. Dupliquer `locales/en.json` → `locales/xx.json`, traduire.
...
| **Landing page** | Pages statiques **par langue** (`/` EN, `/fr/`) avec `hreflang` — le SEO exige du contenu indexable, pas du JS runtime (voir §12) |
...
| Stripe Billing | Standard, frais plus bas (~1,5–2,9 %) | **Toi** = redevable TVA de chaque pays UE → immatriculation **OSS**, facturation conforme, déclarations trimestrielles | Pertinent seulement si volume important ou aversion aux MoR |
...
- [ ] Fonction Edge `billing-webhook` : réception des événements du PSP (souscription créée, paiement échoué, annulation) → mise à jour `guilds.subscription_status`.
...
- [ ] **Documents** (EN + FR) : CGV/CGU, politique de confidentialité, mentions légales, politique cookies (actuellement aucun tracker tiers → pas de bannière nécessaire ; **à préserver** en choisissant une analytics sans cookies, cf. §12.4).
...
- [ ] **RGPD** :
  ...
  - ⚠️ Les pseudos/UID de joueurs sont des données personnelles de **tiers** (les membres ne sont pas tes clients) : le R5 est responsable de son usage, ton rôle = sous-traitant → le préciser dans les CGU.
...
- [ ] **Horaires de rappels par guilde** (le plus critique) : table `guild_event_schedules (guild_id, event_key, day_utc, time_utc, reminder_offsets[], label)` + UI de configuration avec **templates pré-remplis** (les horaires actuels deviennent le template par défaut). `event-reminders` devient un moteur générique qui lit la table. Couvre GvG (créneaux War Prism/War Fortress), SvS (garnison + bataille), Calamity (16 rounds), et les rappels d'événements planifiés (déjà génériques via `start_at`).
...
- [ ] **Paramètres par guilde** (étendre `guild_config`, mécanisme existant) : limites Shadowfront (20 participants + 10 réservistes), seuils de catégories (80/50/20 %), poids de la formule de score (α=6 participation / β=4 performance, bonus Gloire 20 pts, bonus régularité 15 pts @ 80 %), seuil d'alerte récidiviste sanctions (3).
...
- [ ] Conserver les **noms d'événements du jeu** (SvS, GvG, Shadowfront, DTR, Arms Race, Glory) comme vocabulaire produit V1 — c'est le même jeu pour tous les clients. Le renommage libre par guilde est un piège (casse stats/historique) : ne pas l'offrir en V1.
...
- **Aucun logo dans l'UI** (login et sidebar affichent aujourd'hui une image — à remplacer par texte/icône de police).
...
- [ ] 1 logo générique SVG source → déclinaisons PNG (16/32/180/192/512 + maskable avec safe zone 80 %).
...
- [ ] Renommages internes cosmétiques (`window.RAD*` → `window.GMT*`, clés `localStorage rad_*` → `gmt_*` avec migration) : **non bloquants**, à faire opportunément pendant le chantier multi-tenant.
...
6. **App install** : *« Works like an app »* — installable PWA iOS/Android, push notifications, no app store needed.
7. **Pricing** (carte unique) : 9,99 €/month per guild · unlimited members & officers · all features · cancel anytime · 14-day free trial.
8. **FAQ** (6–8 questions) : Which game is it for? · Do my members need accounts? (non — seuls R5/R4 se connectent) · Can I cancel anytime? · What happens to my data if I stop paying? (read-only 30 j, export, purge) · Is my data safe? (isolation par guilde, hébergement UE) · What languages are supported? (EN/FR, more coming) · Can I export my data?
...
- [ ] Performance : Lighthouse ≥ 90 mobile, images WebP/AVIF lazy, fonts en `font-display: swap` (idéalement auto-hébergées — supprime aussi la dépendance Google Fonts côté RGPD).
...
- [ ] Analytics **sans cookies** (Plausible/Umami self-host léger) → pas de bannière consentement, page conversion mesurable.
...
1. **Signup** e-mail + mot de passe → vérification e-mail.
...
5. **Checklist de démarrage** dans l'overview (3 items : membres importés ✓, webhook testé ✓, premier événement lancé ✓) — disparaît une fois complétée.

- [ ] E-mails du cycle d'essai : bienvenue (J0), astuce (J3), rappel fin d'essai (J-3), dernier jour (J14). Prestataire : Resend/Postmark (free tier suffisant au début).
...
- [ ] Remplacer le cache-busting manuel `?v=N` par un micro-build (hash des assets) — source d'erreurs humaines récurrente.
...
- [ ] **Support** : e-mail support@ + **serveur Discord de support** (culturellement idéal pour la cible ; canal #announcements pour les maintenances). Docs : 8–10 articles courts avec captures (créer sa guilde, importer, webhook, horaires, comptes R4, facturation, export, suppression).
...
Frais variables : MoR ~5 % + ~0,50 €/transaction → **~8,9 € nets par abonné**.
...
|-------|---------|-------|-----------------|
| **P0** | Sécurité immédiate (§4) + repo source de vérité | 1–2 j | Prod actuelle sécurisée |
| **P1** | Multi-tenancy complet (§5) + migration RAD | 2–3 sem | 2 guildes de test isolées sur staging |
| **P2** | Auth R5 e-mail + reset + hash R4 + rôles serveur (§6) | 1 sem | Signup/login/reset fonctionnels |
| **P3** | i18n cible (§7) : EN défaut, locales JSON, sélecteur dynamique | 3–4 j | `de.json` de test ajouté sans toucher au code |
| **P4** | Configurabilité (§10) : horaires par guilde, paramètres, événements activables | 1–1,5 sem | Une guilde fictive avec planning différent reçoit les bons rappels |
...
| **Lancement** | Bêta fermée 3–5 guildes (2 sem) → ouverture | — | — |
...
- **Acquisition** : visiteurs landing → inscriptions essai (cible > 5 %), source (Discord communautaires, Reddit du jeu, bouche-à-oreille).
- **Activation** : % d'essais qui importent ≥ 10 membres ET lancent ≥ 1 événement la première semaine (le vrai prédicteur de conversion).
- **Conversion essai → payant** (cible 25–40 % sur cette cible de niche).
...
| Churn d'impayés mal géré → perte de données client | Faible | Élevé | Lecture seule 30 j + export + purge à 60 j seulement |
...
| `index.html` | login : `.gm-avatar-xl` | `<img apple-touch-icon>` (logo **dans l'UI**) | Supprimer l'image → icône police neutre |
...
- [ ] support@ opérationnel + Discord de support ouvert ; docs (8–10 articles) publiées
